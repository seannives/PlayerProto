define(['jquery','d3'], function ($,d3) {
	...
	return {
		...
		init: function init(config, eventManager)
		{
			...
			this.id = config.id || "labelgroupid1";
			/**
		...
		draw : function(container, size)
		{
			...
			*/
			...
			// todo - fix this just like in captionedImage
			labelCollection.attr("transform", function (d, i)  {
							return that.attrFnVal("translate", that.lastdrawn.xScale(d.xyPos[0]),
...
attrFnVal : function attrFnVal(fnName)
{
	...
	return fnCallStr;
		...
		setOpacity : function (opacity, duration, delay)
		{
